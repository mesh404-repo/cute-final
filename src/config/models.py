...
class CacheConfig(BaseModel):
    ...
    # OpenAI-specific caching options
    extended_retention: bool = Field(
        default=True, 
        description="Enable extended cache retention (up to 24h for OpenAI models)"
    )
    cache_key: Optional[str] = Field(
        default=None,
        description="Optional cache key for high-traffic scenarios (>15 req/min)"
    )
...
class AgentConfig(BaseModel):
    ...
    model: str = Field(
        ...
        description="Model to use (litellm: openrouter/provider/model for OpenRouter)"
    )
